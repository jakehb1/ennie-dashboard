...
@app.route('/')
def dashboard():
    drafts = [d for d in real_emails if d['status'] == 'pending']
    
    # Calculate stats
    event_count = len([d for d in drafts if 'event' in d['classification']])
    healing_count = len([d for d in drafts if 'healing' in d['classification']])
